var NCILeftNavWidget = function () {
    ...
    this.BindTemplate = function (templateUri, data, targetDiv) {
        new Akumina.Digispace.AppPart.Data().Templates.ParseTemplate(templateUri, data).done(function (html) {
            $("#" + targetDiv).html(html);
            Akumina.Digispace.AppPart.Eventing.Publish('/widget/loaded/', { properties: _cur.properties });
        });
    };
            ...
            while (levelOccupied < 5) {
                ...
                returnData.forEach((result) => {
                    if (result.NodeLevel == levelOccupied - 1) {
                        ...
                        intNode.IsSelectedNode = (result.Id == targetNavNode.Id ? true : false);
                        currentResultSet.push(intNode);
                    }
                });
                for (let resultSetIdx = 0; resultSetIdx < currentResultSet.length; resultSetIdx ++){
                    ...
                    if (parentNode.NodeLevel == levelOccupied - 1) {
                        for (let navIdx = 0; navIdx < navData.length; navIdx ++) {
                            if (navData[navIdx].ParentItemId == parentNode.Id) {
                                ...
                                currentNavNode.IsSelectedNode = (navData[navIdx].Id == targetNavNode.Id ? true : false);
                        ...
                        if (childNodeArray.length > 0) {
                            ...
                            newReturnArrayWithChildren = firstArrayPart.concat(childNodeArray);
    ...
    this.FetchNavData = function() {
        ...
        if (null != navData) {
            ...
            spCaller.GetList(spRequest).then(function(listData) {
                ...
                while (listEnumerator.moveNext()) {
                    ...
                    var linkUrl = (listItem.get_item('Link') && listItem.get_item('Link').get_url() ? listItem.get_item('Link').get_url() : "");
                ...
                def.resolve(navData);
                Akumina.AddIn.Cache.Set(_cacheKeyForNavData, navData, _cur.GetCacheInterval);
                console.info("Navigational data retrieved from \"" + spRequest.listName + "\" and added to the Akumina Cache.");
            }, function (error) {
    ...
    this.GetCacheKey = function (key) {
        return Akumina.Digispace.ConfigurationContext.getCacheKey(_cur.properties.SenderId + ":" + Akumina.Digispace.PageContext.PageId + ":" + key);
    };
    ...
    this.Render = function () {
        _cur.FetchNavData().then(function(navData) {
            ...
            if (currentPageRelUrl.substring("#") != -1) {
                currentPageRelUrl = location.href.substring(currentPageRelUrl.indexOf("#") + 1);
            }
            ...
            if (!_cur.properties.EditMode) {
                _cur.BindTemplate(_cur.properties.DisplayTemplateUrl, dataForView, _cur.properties.SenderId);
            }    
        }); 
    };
    ...
    this.SetDefaultsProperties = function (requestIn) {
        ...
        requestOut.CaseSensitiveUrlMatching = _cur.GetPropertyValue(requestIn, "casecasesensetiveurlmatching", "");
